...
//Draw image on the page in the specified location and with required size
    page.graphics.drawImage(PdfBitmap(await _readImageData('pet-care.png')),
        const Rect.fromLTWH(180, 15, 120, 100));
...
//Draw the grid in PDF document page
    ...
    layoutResult1 = textElement1.draw(
        page: page1, bounds: const Rect.fromLTWH(0, 0, 0, 0))!;
...
//Draw the grid in PDF document page
    ...
    layoutResult2 = textElement2.draw(
        page: page2, bounds: const Rect.fromLTWH(0, 0, 0, 0))!;
...
//Add rows to grid
    ...
    row61.cells[0].value = 'Según usted, ¿Qué es tenecia responsable?';
...
//Add rows to grid
    ...
    //File file = File('${'Solicitud' + '-' + datosA.nombreCom}.pdf');
    //Write PDF data
    //await file.writeAsBytes(bytes, flush: true);
    //Open the PDF document in mobile
    //OpenFile.open('$path/${'Solicitud' + '-' + datosA.nombreCom}.pdf');
    //Open the PDF document in app web
    // SfPdfViewer.file(file);
    //Download the output file
    AnchorElement(
        href:
            "data:application/octet-stream;charset=utf-16le;base64,${base64.encode(bytes)}")
      ..setAttribute("download", "${'Solicitud' + '-' + datosC.nombreCom}.pdf")
      ..click();
  }
